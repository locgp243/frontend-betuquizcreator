...
const Login = (props) => {
  ...
  const handleLogin = async () => {
    ...
    console.log("Check res login: ", res.user);
    // if (res !== null) {
    //   //const tokenDecode = jwtDecode(res.token);
    //   //setToken(tokenDecode);
    // }

    localStorage.setItem("token", res.token);
    localStorage.setItem("accountId", res.user.accountId);
    ...
    if (res.user.avatar) {
      ...
      const avatarPath = res.user.avatar;
  ...
  return (
    <div className="form_container mx-auto">
      ...
      <div className="title_container">
        <p className="title">Đăng nhập vào BETU Quiz Creator</p>
        <span className="subtitle">
          Hãy đăng nhập để trải nghiệm các tính năng trong trang web của tôi
        </span>
      </div>
      <br />
      ...
      <button className="sign-in_btn" onClick={() => handleLogin()}>
        <span>Đăng nhập</span>
      </button>
      <div className="separator">
        <span className="note">Hoặc</span>
      </div>
      <button className="sign-in_apl">
        <svg
